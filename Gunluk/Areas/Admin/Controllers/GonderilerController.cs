...
namespace Gunluk.Areas.Admin.Controllers
{
    ...
    public class GonderilerController : Controller
    {
        ...
        {
            ViewBag.Mesaj = durum == "eklendi" ? "Gönderi başarıyla oluşturuldu" : durum == "duzenlendi" ? "Gönderi başarıyla güncellendi" :
            durum == "silindi" ? "Gonderi başarıyla silindi" : null;
        ...
        {
            ViewBag.Kategoriler = _db.Kategoriler.Select(x => new SelectListItem() 
            { Value = x.Id.ToString(), Text = x.Ad }).ToList();
        }
